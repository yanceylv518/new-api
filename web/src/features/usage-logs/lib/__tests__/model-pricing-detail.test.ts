...
import {
  ...
  getRecordedDiscountAmounts,
} from '../model-pricing-detail'
...
describe('common log user-model pricing details', () => {
  test('reads exact recorded amounts including zero costs without deriving historical prices', () => {
    assert.deepEqual(
      getRecordedDiscountAmounts({
        quota_before_discount: 100,
        quota_after_discount: 81,
        discount_quota: 19,
      }),
      { before: 100, after: 81, savings: 19 }
    )
    assert.deepEqual(
      getRecordedDiscountAmounts({
        quota_before_discount: 0,
        quota_after_discount: 0,
        discount_quota: 0,
      }),
      { before: 0, after: 0, savings: 0 }
    )
    for (const other of [
      null,
      {},
      { user_model_discount: 0.8 },
      { quota_before_discount: 100, quota_after_discount: 80 },
      {
        quota_before_discount: 100,
        quota_after_discount: 81,
        discount_quota: 20,
      ...
      {
        quota_before_discount: Number.NaN,
        quota_after_discount: 0,
        discount_quota: 0,
      },
    ]) {
      assert.equal(getRecordedDiscountAmounts(other), null)
    }
  })
