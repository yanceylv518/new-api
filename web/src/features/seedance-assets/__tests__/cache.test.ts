...
import { upsertSeedanceAssetInList } from '../lib/cache.ts'
...
describe('Seedance asset query cache', () => {
  test('replaces a manually refreshed asset and preserves sibling assets', () => {
    ...
    const result = upsertSeedanceAssetInList(current, updated)
  ...
  test('adds an uploaded asset to an already loaded empty group', () => {
    ...
    const result = upsertSeedanceAssetInList({ success: true, data: [] }, asset)

    assert.deepEqual(result?.data, [asset])
  })
})
