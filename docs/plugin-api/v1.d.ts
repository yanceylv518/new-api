...
/** 定价字段的适用条件只能引用同一 schema 中的无条件枚举。 */
export type UsageFieldCondition = {field: string; values: readonly string[]; enum?: readonly string[]};
/** displayOrder 为有符号 32 位整数，默认零；价格列按升序展示，同序时按字段名排序。 */
export type UsageFieldSchema = {displayOrder?: number; when?: readonly UsageFieldCondition[]} & (
...
export interface TaskView {task_id: string; platform?: string; action?: string; status: string; progress?: string; fail_reason?: string; created_at?: number; updated_at?: number; data?: unknown; properties?: Record<string, unknown>}
...
/** 为已有任务管理操作构造由宿主鉴权的请求。 */
export declare function buildTaskActionRequest(ctx: TaskQueryContext & {operation: string}): RequestDescriptor;
/** 归一化厂商管理操作成功响应；unknown 要求宿主验证取消状态，禁止直接退款。 */
export declare function parseTaskActionResponse(ctx: TaskQueryContext & {operation: string; status: NormalizedTaskResult["status"]}, response: {statusCode: number; body: unknown}): {action: "cancelled" | "deleted" | "unknown"};
