...
describe('user model discount helpers', () => {
  // 任务美元单价不除以百万，折扣与分组只各乘一次；文本与任务保留各自单位。
  test('discounts task and token unit prices exactly once', () => {
    const options = {
    ...
    assert.equal(
      formatTaskUsageUnitPrice(5, options),
      formatTaskUsageUnitPrice(8, { tokenUnit: 'M' })
    )
    assert.equal(
      formatDynamicUnitPrice(5, options),
      formatDynamicUnitPrice(8, { tokenUnit: 'M' })
    )
    assert.equal(
      formatTaskUsageUnitPrice(5, { ...options, discountMultiplier: 1 }),
      formatTaskUsageUnitPrice(10, { tokenUnit: 'M' })
    )
  })
