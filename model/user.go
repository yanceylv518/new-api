...
type User struct {
	...
	ModelPricingVersion int64                      `json:"-" gorm:"type:bigint;not null;default:1;column:model_pricing_version"`
...
func (user *User) UpdateWithTx(tx *gorm.DB, updatePassword bool) error {
	...
	if err = tx.Model(&current).Omit("quota", "used_quota", "request_count", "auth_version", "model_pricing_version").Updates(newUser).Error; err != nil {
		return err
	}
