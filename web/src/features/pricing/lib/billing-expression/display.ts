...
function taskTier(
  ...
  // 免费数量是管理员定价的一部分，保留在表达式中以便任务冻结同一计费规则。
  ...
  for (let term of flattenBinary(node.args[1], '+')) {
    ...
    let quantity = term.left
    let freeAllowance = 0
    if (quantity.kind === 'call' && quantity.name === 'max') {
      const [difference, minimum] = quantity.args
      if (
        quantity.args.length !== 2 ||
        difference?.kind !== 'binary' ||
        difference.operator !== '-' ||
        minimum?.kind !== 'literal' ||
        minimum.value !== 0 ||
        difference.right.kind !== 'literal' ||
        typeof difference.right.value !== 'number' ||
        !Number.isSafeInteger(difference.right.value) ||
        difference.right.value < 0
      ) {
        return null
      }
      freeAllowance = difference.right.value
      quantity = difference.left
    }
    if (
      quantity.kind !== 'call' ||
      quantity.name !== 'u' ||
      quantity.args.length !== 1 ||
      quantity.args[0].kind !== 'literal' ||
      typeof quantity.args[0].value !== 'string'
    ) {
      return null
    }
    const field = quantity.args[0].value
    const definition = schema[field]
    if (
    ...
    if (freeAllowance > 0 && definition.unit !== 'count') return null
    if (freeAllowance > 0) freeAllowances[field] = freeAllowance
  ...
  return {
    ...
    constant,
    unitPrices,
    ...(Object.keys(freeAllowances).length ? { freeAllowances } : {}),
  }
}
