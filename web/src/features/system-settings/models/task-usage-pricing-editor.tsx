...
import { TaskFreeAllowanceInput } from './task-free-allowance-input'
...
function TaskBillingPreview(props: TaskBillingPreviewProps) {
  ...
  const enumFields = getTaskEnumFields(props.usageSchema, props.sample)
  const numberFields = getTaskNumberFields(props.usageSchema, props.sample)
...
export const TaskUsagePricingEditor = memo(function TaskUsagePricingEditor(
  ...
  const [previewSampleState, setPreviewSample] = useState<
  ...
  // 从当前操作派生合法用量，无需 effect 追改选择器和隐藏输入。
  const previewSample = normalizeTaskUsageSample(
    props.usageSchema,
    previewSampleState
  )
  ...
  const handleFillColumn = (priceKey: string, value: number) => {
    const nextRows = matrixRows.map((row) => {
      ...
      if (!getTaskUsageField(props.usageSchema[priceKey], row.combination)) {
        return row
      }
      return {
        ...row,
          ...
          <>
            {enumFields.length > 0 ? (
              <div className='flex flex-col gap-3'>
                <p className='text-muted-foreground text-xs'>
                  {t(
                    'Set prices and free quantities for each condition. Only usage above the free quantity is charged. Token prices are per million tokens.'
                  )}
                </p>
                <TaskPricingMatrix
                        ...
                        return (
                          <Field key={field} className='gap-1.5'>
                            ...
                            {definition.unit === 'count' && (
                              <TaskFreeAllowanceInput
                                label={description}
                                value={
                                  matrixRows[0].freeAllowances?.[field] ?? 0
                                }
                                onChange={(value) =>
                                  handleRowChange(0, {
                                    ...matrixRows[0],
                                    freeAllowances: {
                                      ...matrixRows[0].freeAllowances,
                                      [field]: value,
                                    },
                                  })
                                }
