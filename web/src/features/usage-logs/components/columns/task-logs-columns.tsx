...
import { TaskVideoPreview } from '../dialogs/task-video-preview'
...
// 使用任务结果地址判断视频是否可预览，错误原因只保留给失败任务展示。
export function TaskLogDetailsCell({ log }: { log: TaskLog }) {
  const { t } = useTranslation()
  ...
  // 新任务使用 result_url，旧任务仍可能把结果地址写在 fail_reason 中。
  const videoURL =
    log.result_url?.trim() ||
    (failReason.trim().startsWith('http') ? failReason.trim() : '')
  const hasResultUrl = Boolean(videoURL)

  if (isSuccess && isVideoTask && hasResultUrl) {
    return <TaskVideoPreview taskId={log.task_id} videoUrl={videoURL} />
  }
...
export function useTaskLogsColumns(isAdmin: boolean): ColumnDef<TaskLog>[] {
  ...
  columns.push(
    ...
    createProgressColumn<TaskLog>({ headerLabel: t('Progress') }),
    {
      ...
      header: t('Details'),
      cell: ({ row }) => <TaskLogDetailsCell log={row.original} />,
      size: 200,
      maxSize: 220,
    }
  )
