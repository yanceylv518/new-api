...
- 生成请求引用 `asset://<AssetId>` 时，网关会按当前用户校验本地素材，并将请求
  固定到素材所属的上游渠道和账号；同一请求引用多个素材时，它们必须属于同一
  上游账号。重试也沿用该账号，不会重新随机选择其他素材账号。Token 的显式渠道
  绑定优先级高于素材亲和；两者冲突时请求直接失败，避免绕过 Token 的渠道权限。
- 素材所属渠道不提供请求模型时，网关会在提交前返回素材账号与模型不匹配；素材
  不存在、已删除或不属于当前用户时，也不会把 `asset://` 当成普通 URL 继续发送。
- 缺少账号指纹的历史素材无法安全确定上游密钥，网关会拒绝其生成请求，避免随机
  选中其他账号造成“素材尚未同步到可用账号”。
