...
func RecalculateTaskQuotaByTokens(ctx context.Context, task *model.Task, totalTokens int) bool {
	quota, reason, clamp, amounts, ok := taskQuotaByTokens(task, totalTokens)
	if ok {
		RecalculateTaskQuotaWithAmounts(ctx, task, quota, amounts, reason, clamp)
	}
	return ok
}

// taskQuotaByTokens 只计算额度，供原有结算和视频原子终态事务共用同一倍率规则。
func taskQuotaByTokens(task *model.Task, totalTokens int) (int, string, *common.QuotaClamp, *types.DiscountAmounts, bool) {
	if totalTokens <= 0 {
	...
	return actualQuota, reason, clamp, types.NewDiscountAmounts(originalQuota, actualQuota), true
}
