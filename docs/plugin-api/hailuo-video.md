...
`base_video` 的宽高须能被 32 整除，必须包含音轨，帧数为 107–362、步长 17。
额外参考视频的帧率范围为 23.976–60 FPS。媒体还需符合模型的其他输入要求，
素材校验可能在受理后进行，失败原因由任务查询结果返回。

媒体地址必须在上游读取时仍可访问。签名有效期、权限和上游网络访问限制可能
导致下载失败。若返回 `media URL cannot access a private address`，应检查地址
及重定向目标，或转存至上游可访问的对象存储。已有符合条件的任务时优先使用
源任务 ID。受理成功不代表素材校验通过，应继续查询终态及退款记录。

### 回调行为

收到 `challenge` 验证请求时，接收端应以 JSON 原样返回其值。回调验证与任务
受理分开处理：创建接口不以回调地址可达性作为受理前提，返回任务 ID 不表示
回调验证成功或通知已送达。客户端需处理重复、乱序通知，避免状态回退，
并保留任务查询以获取最终结果。
...
输出费用按上游返回的 `output_seconds` 结算。例如文件长 5.166667 秒，而
`output_seconds=6` 时，按 6 秒计费。若配置 $0.30/秒且无其他费用，净扣为
$1.80；若预扣 $4.50，则完成后退回 $2.70。每个任务以上游实际计费用量为准，
价格由管理员配置，预扣和媒体文件时长不等同于最终费用。
