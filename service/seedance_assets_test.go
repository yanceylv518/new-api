...
func TestSeedanceMutationsValidateBusinessResponses(t *testing.T) {
	for _, body := range []string{
		...
		t.Run(body, func(t *testing.T) {
			...
			client, err := NewSeedanceAssetClient(&model.Channel{BaseURL: &server.URL, Key: "fixture-key", Status: common.ChannelStatusEnabled})
...
func TestSeedanceProxyRequestHasIndependentDeadline(t *testing.T) {
	...
	client, err := NewSeedanceAssetClient(&model.Channel{BaseURL: &baseURL, Setting: &settings, Key: "fixture-key", Status: common.ChannelStatusEnabled})
	require.NoError(t, err)
	_, _, err = client.GetSeedanceAsset(t.Context(), "asset")
	require.NoError(t, err)
...
func TestSeedanceClientBindsSelectedAccountAcrossKeyReordering(t *testing.T) {
	...
	t.Cleanup(server.Close)
	channel := &model.Channel{BaseURL: &server.URL, Key: "disabled-key\nselected-key", Status: common.ChannelStatusEnabled,
...
func TestSeedanceAssetClientUsesDocumentedIDFields(t *testing.T) {
	...
	t.Cleanup(server.Close)
	baseURL := server.URL
	client, err := NewSeedanceAssetClient(&model.Channel{BaseURL: &baseURL, Key: "test-key", Status: common.ChannelStatusEnabled})
	require.NoError(t, err)
