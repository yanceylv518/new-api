...
# Administrator-configured free quantity per task for a count-unit field
tier("images", max(u("input_images") - 5, 0) * 0.2)
...
Count-unit terms may replace `u("field")` with `max(u("field") - N, 0)`, where
`N` is a non-negative safe integer chosen by the administrator. The visual
editor preserves this allowance per field and tier, including at zero price;
different allowances prevent rows from collapsing into a uniform price. Existing
linear terms mean zero allowance. Plugins continue to provide total quantities.
This uses the existing expression runtime and frozen task snapshot; no additional
setting, database column, or second discount calculation is introduced.
