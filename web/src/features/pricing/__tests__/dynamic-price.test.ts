...
describe('task dynamic pricing', () => {
  ...
  // 视频主价格始终排在附加素材费用前，即使视频本身免费也不能隐藏它。
  test.each([0, 0.5])(
    'prioritizes the video price %s ahead of paid input images',
    (videoPrice) => {
      const summary = getDynamicPricingSummary(
        pricingModel({
          billing_mode: 'tiered_expr',
          billing_expr: `tier("video", u("input_images") * 0.2 + u("input_video_seconds") * 0 + u("seconds") * ${videoPrice})`,
          billing_usage_schema: {
          ...
          billing_usage_examples: [
            {
              label: '5s with 9 images',
              facts: { seconds: 5, input_images: 9, input_video_seconds: 0 },
            },
          ],
        }),
        summaryOptions
