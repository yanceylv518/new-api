...
// 密钥只用于提交，保存成功后清空表单中的敏感值，不从配置接口回填。
export function PrivateAssetOSSSettingsSection(
  ...
  const mutation = useMutation({
    mutationFn: updatePrivateAssetOSSSettings,
    gcTime: 0,
  })
  const form = useForm<PrivateAssetOSSFormValues>({
    resolver: zodResolver(
      createPrivateAssetOSSSchema(t, props.secretConfigured)
    ),
  ...
  const onSubmit = async (values: PrivateAssetOSSFormValues) => {
    ...
    try {
      ...
      // 通用错误处理器会输出完整 Axios 请求；这里只显示消息，避免把密钥写入控制台。
      toast.error(
