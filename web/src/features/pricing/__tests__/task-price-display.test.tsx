...
it('orders task prices consistently in overview and group pricing tables', () => {
  ...
  const labels = [
    'Video price',
    'Context input price',
    'Context output price',
    'Image price',
    'Input video price',
    'Additional charge',
  ]
  const schema: BillingUsageSchema = {
    operation: { enum: ['generation', 'context_ir'] },
    completion_tokens: {
      type: 'number',
      unit: 'token',
      displayOrder: 30,
  ...
  for (const table of tables) {
    const headings = within(table)
      ...
      .filter((heading) =>
        labels.some((label) => heading.textContent?.includes(label))
      )
...
// 标准价格明细保留每个条件的免费数量，移动端和桌面端不能把超额单价显示成全部用量收费。
it('shows the free quantity alongside an excess-image unit price', () => {
  render(
    <DynamicPricingBreakdown
      billingExpr='tier("image", max(u("input_images") - 5, 0) * 0.2)'
      usageSchema={{
        input_images: {
          type: 'number',
          unit: 'count',
  ...
  expect(screen.getAllByText('Free quantity per task: 5')).toHaveLength(2)
  expect(
    screen.queryByText('Special billing expression')
  ).not.toBeInTheDocument()
})

// 摘要应与普通视频生成示例一致，不受表达式分支顺序影响，也不额外展示档位标签。
it('prioritizes H3 video prices without showing Context-IR prices or tier labels on cards', async () => {
  const h3Model: PricingModel = {
    ...model,
    model_name: 'MiniMax-H3',
    billing_expr:
      'u("operation") == "context_ir" ? tier("context_ir", u("prompt_tokens") * 2 / 1000000 + u("completion_tokens") * 8 / 1000000) : u("operation") == "generation" && u("resolution") == "768P" ? tier("video_768", u("seconds") * 0.5) : u("operation") == "generation" && u("resolution") == "2K" ? tier("video_2k", u("seconds") * 0.8) : tier("regeneration", u("seconds") * 0.3)',
    billing_usage_schema: {
      operation: {
  ...
  const view = render(<ModelCard model={h3Model} onClick={vi.fn()} />)
  expect(screen.getByText('Video generation unit price')).toBeVisible()
  expect(screen.getByText('$0.5 – $0.8')).toBeVisible()
  expect(screen.getByText('H3 generation 768P 5s ≈ $2.5')).toBeVisible()
  expect(
    screen.queryByText(/context_ir|H3-Context-IR|\(regeneration\)/)
  ).not.toBeInTheDocument()
  expect(screen.queryByText('Input image unit price')).not.toBeInTheDocument()
  await act(() => i18next.changeLanguage('zhCN'))
  expect(screen.getByText('视频生成单价')).toBeVisible()
  expect(screen.queryByText(/context_ir|H3-Context-IR/)).not.toBeInTheDocument()
  // 表格视图复用同一价格摘要；详情仍保留独立计价的所有操作。
  view.rerender(<ModelPriceCell model={h3Model} />)
  expect(view.container).toHaveTextContent('0.5 – 0.8/s')
  expect(view.container).toHaveTextContent('USD')
  expect(
    getTaskPricingDisplayTiers(
      h3Model.billing_expr,
      h3Model.billing_usage_schema
    )
  ).toEqual(
    expect.arrayContaining([
      expect.objectContaining({
        conditions: [{ field: 'operation', value: 'context_ir' }],
        unitPrices: expect.objectContaining({
          prompt_tokens: 2,
          completion_tokens: 8,
        }),
      }),
...
// 无条件的档位没有可展示的操作信息，隐藏仅供表达式追踪的内部名称。
it('omits internal tier names when task pricing has no conditions', () => {
  render(
    <ModelCard
      model={{
        ...model,
        billing_expr: 'tier("internal_price", u("songs") * 0.22)',
        billing_usage_schema: {
          songs: {
            type: 'number',
            unit: 'count',
            description: 'Song generation unit price',
          },
        },
        billing_usage_examples: [],
      }}
  ...
  expect(screen.getByText('Song generation unit price')).toBeVisible()
  expect(screen.queryByText('(internal_price)')).not.toBeInTheDocument()
})
