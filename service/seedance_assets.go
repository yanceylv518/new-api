...
func StoreSeedanceAssetUpload(ctx context.Context, reader io.Reader, filename, contentType, declaredType string, size int64, userID int) (*SeedanceAssetUpload, error) {
	...
	if err := storage.UploadPrivateAsset(ctx, objectKey, detectedContentType, written, storedFile); err != nil {
		if cleanupErr := RollbackSeedanceAssetObject(ctx, objectKey); cleanupErr != nil {
...
func RemoveSeedanceAssetObject(ctx context.Context, objectKey string) error {
...
// RollbackSeedanceAssetObject 在原请求取消后仍使用有限时间回收未入库的 OSS 对象。
func RollbackSeedanceAssetObject(ctx context.Context, objectKey string) error {
	cleanupCtx, cancel := seedanceAssetCleanupContext(ctx)
	defer cancel()
	return RemoveSeedanceAssetObject(cleanupCtx, objectKey)
}
...
func RollbackSeedanceAsset(ctx context.Context, client *SeedanceAssetClient, assetID, objectKey string) error {
	cleanupCtx, cancel := seedanceAssetCleanupContext(ctx)
	defer cancel()
	if client == nil {
	...
	if strings.TrimSpace(objectKey) != "" {
		if err := RemoveSeedanceAssetObject(cleanupCtx, objectKey); err != nil {
...
func SeedanceAssetOSSPreviewURL(ctx context.Context, objectKey string) (string, error) {
	storage, err := privateAssetOSSStorageFactory()
	if err != nil {
		return "", err
	}
...
// NewSeedanceAssetClient 从已启用的 DoubaoVideo 渠道创建客户端。
func NewSeedanceAssetClient(channel *model.Channel) *SeedanceAssetClient {
...
func (client *SeedanceAssetClient) call(ctx context.Context, action string, request any, response any) error {
	...
	url := fmt.Sprintf("%s%s?Action=%s&Version=2024-01-01", client.baseURL, client.path, action)
	...
	return common.DecodeJson(resp.Body, response)
}
...
func (response seedanceAssetResponse) businessError() error {
	...
	return fmt.Errorf("seedance asset API request failed: %s", message)
}
...
func (client *SeedanceAssetClient) DeleteSeedanceAsset(ctx context.Context, assetID string) error {
	return client.call(ctx, "DeleteAsset", map[string]string{"Id": assetID}, &struct{}{})
}
...
func (client *SeedanceAssetClient) DeleteSeedanceAssetGroup(ctx context.Context, groupID string) error {
	return client.call(ctx, "DeleteAssetGroup", map[string]string{"Id": groupID}, &struct{}{})
}
...
// FindSeedanceAssetChannel 选择当前系统第一个可用的 Seedance 渠道。
func FindSeedanceAssetChannel() (*model.Channel, error) {
	channels, err := model.GetChannelsByType(0, 100, true, constant.ChannelTypeDoubaoVideo)
	if err != nil {
		return nil, err
	}
	for _, channel := range channels {
		if channel.Status == common.ChannelStatusEnabled && channel.GetBaseURL() != "" {
			// 列表查询会主动省略密钥，必须重新读取完整渠道后再调用上游。
			fullChannel, err := model.GetChannelById(channel.Id, true)
			if err == nil && fullChannel.Key != "" {
